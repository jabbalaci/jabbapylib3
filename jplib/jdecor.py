...
# from jplib.jdecor import jd_time
# from jplib.jdecor import jd_requires
"""
...
def jd_time(func):
    ...
    def wrapper(*arg):
        start = time.time()
        print('# dec_time: enter', func.__name__)
        try:
            return func(*arg)
        finally:
            print('# dec_time: exit {} ({} sec.)'.format(func.__name__, time.time() - start))
    #
    return wrapper
